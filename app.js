// app.js — NexariOS v7.2
// Patch: loadSearch now passes the active date so search
// results match the currently viewed day (not always today).
...
async function loadSearch(query) {
  ...
  try {
    ...
    // PATCH v7.2: pass the active date so search matches the
    // day the user is currently viewing, not always today.
...
function toggleGameLineups(li, game) {
  ...
  const homePitcher = game.probableHomePitcher || "TBD";
  const awayPitcher = game.probableAwayPitcher || "TBD";
  ...
  inner.innerHTML = `
    <div class="lineup-columns">
      <div class="lineup-col">
        ...
        <div class="player-row" style="opacity:0.6; font-size:11px;">
          <span>SP: ${homePitcher}</span>
        </div>
        ${homeList || `<div class="player-row empty-state">No roster data.</div>`}
      </div>
      <div class="lineup-col">
        <div class="lineup-title">${game.awayTeam || game.away || ""}</div>
        <div class="player-row" style="opacity:0.6; font-size:11px;">
          <span>SP: ${awayPitcher}</span>
        </div>
        ${awayList || `<div class="player-row empty-state">No roster data.</div>`}
      </div>
    </div>
  `;
...
function renderAccuracy() {
  ...
  hrRbiEl.innerHTML = `
    <div class="metric-row"><span>Last 7</span><span>${last7}%</span></div>
    <div class="metric-row"><span>Last 30</span><span>${last30}%</span></div>
    <div class="metric-row">
      <span>HR Hitters Today</span>
      <span>${hrHitters.join(", ") || "None yet"}</span>
    </div>
  `;
